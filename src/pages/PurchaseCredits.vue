<template>
  <section class="container w-full h-full flex justify-center items-center flex-col">
    <div class="bg-white p-20 rounded-xl flex justify-center flex-col items-center">
      <h5 class="text-primary font-semibold text-xl">
        Compra Créditos con tu Tarjeta de Debito/Crédito
      </h5>
      <p class="w-96 text-center font-medium text-sm">
        Sistema de pagos seguro a través de Stripe. Tus datos están protegidos. Si tienes algún inconveniente, contacta
      ...
      <div class="mt-4 mb-4 grid justify-items-center">
        <form action="" method="post" class="w-72 sm:w-96 space-y-2">
          ...
          <select name="amount" id="amount" v-model="amount" class="w-full border-secondary border-2 rounded p-2">
          ...
          <div class="grid grid-cols-5 w-full gap-3">
            <div id="card-number-element" class="col-span-3 w-auto border p-2.5 rounded"></div>
        ...
        <button @click="placeOrderButtonPressed" type="submit" v-if="!paymentInProcess"
          class="bg-primary p-2.5 text-white rounded-md mt-2 w-72 sm:w-96">
          Pay
        </button>

        <button class="bg-slate-500 p-2.5 text-white rounded-md mt-2 w-72 sm:w-96" v-if="paymentInProcess" disabled>
          Pay
        </button>
        ...
        <div class="mt-5" unselectable="on">
          <p class="font-medium text-sm flex justify-center items-center gap-1 text-gray-400">
